...
echo "============================================"
echo "  V3 Multi-Level TUNED (mem_block=0.12)"
echo "============================================"
...
for wl_name in "${!MICRO_WL[@]}"; do
    ...
    outdir="${OUTBASE}/v3ml_t1/${wl_name}/latest"
    run_and_mcpat "${outdir}" "v3ml_t1 ${wl_name}" \
...
for bench in "${GAPBS_BENCHMARKS[@]}"; do
    ...
    outdir="${OUTBASE}/v3ml_t1/gapbs_${bench}/latest"
    run_and_mcpat "${outdir}" "v3ml_t1 gapbs_${bench}" \
